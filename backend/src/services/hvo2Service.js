...
export const readDataHvo2 = async (modbusClient, deviceID, deviceLabel) => {
  try {
    ...
    const flows = {
      'Расход умягченной воды после Ф4/1,2,3': (await modbusClient.readFloat(deviceID, 0x0000, deviceLabel) *
        2).toFixed(0),
      'Расход воды в канализацию': (await modbusClient.readFloat(deviceID, 0x0002, deviceLabel) * 0.6).toFixed(0),
    };
    const levels = {
      'Уровень воды в E2/1 (Титан)': (await modbusClient.readFloat(deviceID, 0x0004, deviceLabel) * 60 + 820).toFixed(
        0),
      'Уровень воды в E2/1 (Мида)': (await modbusClient.readFloat(deviceID, 0x0006, deviceLabel) * 60 + 820).toFixed(0),
      'Уровень воды в E2/2 (Титан)': (await modbusClient.readFloat(deviceID, 0x0008, deviceLabel) * 60 + 820).toFixed(
        0),
    ...
    const others = {
      ...
      'Измерение уровня воды на регулятор уровня': (await modbusClient.readFloat(deviceID, 0x003E, deviceLabel) * 60 +
        820).toFixed(0),
    };
